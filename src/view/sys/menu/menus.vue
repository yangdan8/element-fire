    ...
    <section class="menus">
        <Card dis-hover>
            <p slot="title">
                ...
                <!-- {{$store.state.app.activeMenu.title}} -->菜单管理
            </p>
            <TreeView>
...
import MenuEdit from '@/view/sys/menu/menu-edit.vue';
export default {
    ...
    'methods': {
        getData() {
            $http.get('v1/baseMenu/getTreeAll').then(data => {
                ...
                util.recurTreeData(
                    this.data4Tree,
                    node => node.expand && expandIds.push(node.uid)
                );
                // 设置展开节点id
        ...
        renderTreeContent(h, { root, node, data }) {
            ...
            _title = (
                <div class="tree-node-title" onClick={() => this.editNode(data)}>
                    {data.title}
                </div>
            );
            ...
            if (data.type !== '1') {
                _addBtn = (
                    <span class="opt" onClick={() => this.addNode(data.uid)}>
            ...
            return (
                <div
                    class={data.uid === this.activeId ? 'tree-node active' : 'tree-node'}
                >
                    {_title}
                    <div class="tree-options">
                        <span class="opt" onClick={() => this.editNode(data)}>
                        ...
                        <Poptip
                            ...
                            transfer
                            title="确定删除吗"
                            on-on-ok={() => this.delNode(data.uid)}
                        >
                            <span>
    ...
    'components': { TreeView, Poptip, MenuEdit },
    'filters': {},
    'watch': {},
    mounted() {
    // this.getData()
